...
fn check_parse_and_serialize(sdp_str: &str) {
    let parsed_sdp = cloudmedia::protocol::sdp::parse_sdp(sdp_str, true);
...
fn parse_minimal_sdp() {
    ...
    let sdp_res = cloudmedia::protocol::sdp::parse_sdp(sdp_str, true);
    ...
    assert_eq!(
        *msection.get_type(),
        cloudmedia::protocol::sdp::media_type::SdpMediaValue::Audio
    );
    assert_eq!(msection.get_port(), 0);
    assert_eq!(msection.get_port_count(), 0);
    assert_eq!(
        *msection.get_proto(),
        cloudmedia::protocol::sdp::media_type::SdpProtocolValue::UdpTlsRtpSavpf
    );
...
t=0 0\r\n
m=audio 0 UDP/TLS/RTP/SAVPF 0\r\n";
    let sdp_res = cloudmedia::protocol::sdp::parse_sdp(sdp, false);
    assert!(sdp_res.is_ok());
    let sdp_opt = sdp_res.ok();
    assert!(sdp_opt.is_some());
    let sdp = sdp_opt.unwrap();
...
fn parse_minimal_sdp_with_most_session_types() {
    let sdp_str = "v=0\r\n\
                   ...
                   m=audio 0 UDP/TLS/RTP/SAVPF 0\r\n";
    let sdp_res = cloudmedia::protocol::sdp::parse_sdp(sdp_str, false);
    assert!(sdp_res.is_ok());
    let sdp_opt = sdp_res.ok();
    assert!(sdp_opt.is_some());
    let sdp = sdp_opt.unwrap();
...
fn parse_minimal_sdp_with_most_media_types() {
    ...
    assert_eq!(
        *msection.get_type(),
        cloudmedia::protocol::sdp::media_type::SdpMediaValue::Video
    );
    assert_eq!(msection.get_port(), 0);
    assert_eq!(
        *msection.get_proto(),
    ...
    assert!(!msection.get_attributes().is_empty());
    assert!(msection
        .get_attribute(cloudmedia::protocol::sdp::attribute_type::SdpAttributeType::Sendrecv)
        .is_some());
...
a=ssrc:2655508255 cname:{735484ea-4f6c-f74a-bd66-7425f8476c2e}\r\n";
    ...
    assert!(msection
        ...
        .is_some());
    assert!(msection
        .get_attribute(cloudmedia::protocol::sdp::attribute_type::SdpAttributeType::Extmap)
        .is_some());
    assert!(msection
        .get_attribute(cloudmedia::protocol::sdp::attribute_type::SdpAttributeType::Fmtp)
        .is_some());
    assert!(msection
        .get_attribute(cloudmedia::protocol::sdp::attribute_type::SdpAttributeType::IcePwd)
        .is_some());
    assert!(msection
        .get_attribute(cloudmedia::protocol::sdp::attribute_type::SdpAttributeType::IceUfrag)
        .is_some());
    assert!(msection
        .get_attribute(cloudmedia::protocol::sdp::attribute_type::SdpAttributeType::Mid)
        .is_some());
    assert!(msection
        .get_attribute(cloudmedia::protocol::sdp::attribute_type::SdpAttributeType::Mid)
        .is_some());
    assert!(msection
        .get_attribute(cloudmedia::protocol::sdp::attribute_type::SdpAttributeType::Msid)
        .is_some());
    assert!(msection
        .get_attribute(cloudmedia::protocol::sdp::attribute_type::SdpAttributeType::RtcpMux)
        .is_some());
    assert_eq!(
        msection
            .get_attributes_of_type(cloudmedia::protocol::sdp::attribute_type::SdpAttributeType::Rtpmap)
            .len(),
        4
    );
    assert!(msection
        .get_attribute(cloudmedia::protocol::sdp::attribute_type::SdpAttributeType::Setup)
        .is_some());
    assert!(msection
        .get_attribute(cloudmedia::protocol::sdp::attribute_type::SdpAttributeType::Ssrc)
        .is_some());
}
...
a=ssrc:2709871439 cname:{735484ea-4f6c-f74a-bd66-7425f8476c2e}";
    ...
    assert!(!msection.get_attributes().is_empty());
    assert!(msection
        .get_attribute(cloudmedia::protocol::sdp::attribute_type::SdpAttributeType::Recvonly)
        .is_some());
    assert!(!msection
        .get_attribute(cloudmedia::protocol::sdp::attribute_type::SdpAttributeType::Extmap)
        .is_some());
    assert_eq!(
        msection
            .get_attributes_of_type(cloudmedia::protocol::sdp::attribute_type::SdpAttributeType::Fmtp)
            .len(),
        3
    );
    assert!(msection
    ...
    assert_eq!(
        msection
            .get_attributes_of_type(cloudmedia::protocol::sdp::attribute_type::SdpAttributeType::Rtcpfb)
            .len(),
        12
    );
    assert!(msection
...
fn parse_firefox_datachannel_offer() {
    ...
    assert_eq!(
        *msection.get_type(),
        cloudmedia::protocol::sdp::media_type::SdpMediaValue::Application
    );
    assert_eq!(msection.get_port(), 49760);
    assert_eq!(
        *msection.get_proto(),
        cloudmedia::protocol::sdp::media_type::SdpProtocolValue::DtlsSctp
    );
    assert!(msection.get_connection().is_some());
    assert!(msection.get_bandwidth().is_empty());
    assert!(!msection.get_attributes().is_empty());
    ...
    assert!(msection
        ...
        .is_some());
    assert!(msection
        .get_attribute(cloudmedia::protocol::sdp::attribute_type::SdpAttributeType::EndOfCandidates)
        .is_some());
    assert!(msection
        .get_attribute(cloudmedia::protocol::sdp::attribute_type::SdpAttributeType::Mid)
        .is_some());
    assert!(!msection
        ...
        .is_some());
    assert!(!msection
        .get_attribute(cloudmedia::protocol::sdp::attribute_type::SdpAttributeType::Rtcpfb)
        .is_some());
    assert!(!msection
        .get_attribute(cloudmedia::protocol::sdp::attribute_type::SdpAttributeType::RtcpMux)
        .is_some());
    assert!(!msection
        .get_attribute(cloudmedia::protocol::sdp::attribute_type::SdpAttributeType::Rtpmap)
        .is_some());
    assert!(msection
        .get_attribute(cloudmedia::protocol::sdp::attribute_type::SdpAttributeType::Sctpmap)
        .is_some());
    assert!(msection
        .get_attribute(cloudmedia::protocol::sdp::attribute_type::SdpAttributeType::Setup)
        .is_some());
...
a=ssrc:2673335628 label:b6ec5178-c611-403f-bbec-3833ed547c09\r\n";
    let sdp_res = cloudmedia::protocol::sdp::parse_sdp(sdp, true);
    assert!(sdp_res.is_ok());
    let sdp_opt = sdp_res.ok();
    assert!(sdp_opt.is_some());
    let sdp = sdp_opt.unwrap();
