...
use trackable::error::{Failure, Failed, ErrorKindExt};
...
use crate::proto::rtcp::payload_specific_feedback::PayloadSpecificFeedbackPacket;
use crate::proto::rtcp::transport_layer_feedback::TransportLayerFeedbackPacket;
use crate::proto::rtp::constants::RTP_VERSION;
use crate::proto::rtp::traits::RtcpPacketTrait;
use crate::proto::traits::{PacketData, PacketTrait, ReadFrom, ReadPacket, Result, WritePacket, WriteTo};
// use super::traits::*;
...
impl ReadPacket for RtcpPacketReader {
    ...
    fn read_packet<R: Read>(&mut self, reader: &mut R) -> Result<Self::Packet> {
        ...
        while !reader.is_empty() {
            let packet = track_try_unwrap!(RtcpPacket::read_from(reader).map_err(Error::from));
            packets.push(packet);
        }
...
impl ReadFrom for RtcpPacket {
    fn read_from<R: Read>(reader: &mut R) -> Result<Self> {
        let mut buf = [0; 2];
        track_err!(reader.read_exact(&mut buf).map_err(Error::from));
...
pub fn write_sctp<W: Write>(
    ...
    track_try_unwrap!(writer.write_u8(RTP_VERSION << 6 | packet_specific).map_err(Error::from));
    track_try_unwrap!(writer.write_u8(packet_type).map_err(Error::from));

    let word_count = payload.len() / 4;
    track_assert!(word_count < 0x10000, ErrorKind::Invalid);

    track_try_unwrap!(writer.write_u16be(word_count as u16).map_err(Error::from));
    track_try_unwrap!(writer.write_all(payload).map_err(Error::from));
...
mod tests {
    ...
    use crate::proto::rtcp::rtcp_packet::CommonHeader;
    use crate::proto::traits::PacketData;
