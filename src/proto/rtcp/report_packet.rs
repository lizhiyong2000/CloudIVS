...
impl ReadFrom for ReceptionReport {
    fn read_from<R: Read>(reader: &mut R) -> Result<Self> {
        ...
        let fraction_lost = track_try_unwrap!(reader.read_u8().map_err(Error::from));
        let packets_lost = track_try_unwrap!(reader.read_u24be().map_err(Error::from));
        let seq_num_ext = track_try_unwrap!(reader.read_u32be().map_err(Error::from));
        let jitter = track_try_unwrap!(reader.read_u32be().map_err(Error::from));
        let last_sr_timestamp = track_try_unwrap!(reader.read_u32be().map_err(Error::from));
        let delay_since_last_sr = track_try_unwrap!(reader.read_u32be().map_err(Error::from));
...
impl WriteTo for ReceptionReport {
    fn write_to<W: Write>(&self, writer: &mut W) -> Result<()> {
        ...
        track!(writer.write_u8(self.fraction_lost).map_err(Error::from));
        track!(writer.write_u24be(self.packets_lost).map_err(Error::from));
        track!(writer.write_u32be(self.seq_num_ext).map_err(Error::from));
        track!(writer.write_u32be(self.jitter).map_err(Error::from));
        track!(writer.write_u32be(self.last_sr_timestamp).map_err(Error::from));
        track!(writer.write_u32be(self.delay_since_last_sr).map_err(Error::from));
...
impl ReadFrom for SenderReportPacket {
    fn read_from<R: Read>(reader: &mut R) -> Result<Self> {
        let (reception_report_count, payload) = track_try_unwrap!(read_sctp(reader, RTCP_PACKET_TYPE_SR).map_err(Error::from));
        let reader = &mut &payload[..];

        let ssrc = track_try_unwrap!(reader.read_u32be().map_err(Error::from));

        let ntp_sec = track_try_unwrap!(reader.read_u32be().map_err(Error::from));
        let ntp_frac  = track_try_unwrap!(reader.read_u32be().map_err(Error::from));
        let rtp_timestamp = track_try_unwrap!(reader.read_u32be().map_err(Error::from));
        let sent_packets = track_try_unwrap!(reader.read_u32be().map_err(Error::from));
        let sent_octets = track_try_unwrap!(reader.read_u32be().map_err(Error::from));

        let mut reception_reports = Vec::new();
        for _ in 0..reception_report_count {
            let report = track_try_unwrap!(ReceptionReport::read_from(reader).map_err(Error::from));
            reception_reports.push(report);
        }
        let extensions = track_try_unwrap!(reader.read_all_bytes().map_err(Error::from));
...
impl WriteTo for SenderReportPacket {
    fn write_to<W: Write>(&self, writer: &mut W) -> Result<()> {
        ...
        track!((&mut payload).write_u32be(self.ssrc).map_err(Error::from));
        track!((&mut payload).write_u32be(self.ntp_sec).map_err(Error::from));
        track!((&mut payload).write_u32be(self.ntp_frac).map_err(Error::from));
        track!((&mut payload).write_u32be(self.rtp_timestamp).map_err(Error::from));
        track!((&mut payload).write_u32be(self.sent_packets).map_err(Error::from));
        track!((&mut payload).write_u32be(self.sent_octets).map_err(Error::from));
        for report in self.reception_reports.iter() {
            track!(report.write_to(&mut payload).map_err(Error::from));
        }
...
impl ReadFrom for ReceiverReportPacket {
    fn read_from<R: Read>(reader: &mut R) -> Result<Self> {
        let (reception_report_count, payload) = track_try_unwrap!(read_sctp(reader, RTCP_PACKET_TYPE_RR).map_err(Error::from));
        let reader = &mut &payload[..];
