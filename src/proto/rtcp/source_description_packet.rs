...
use crate::proto::rtcp::constants::{*};
...
impl ReadFrom for SourceDescriptionPacket {
    fn read_from<R: Read>(reader: &mut R) -> Result<Self> {
        let (source_count, payload) = track_try_unwrap!(read_sctp(reader, RTCP_PACKET_TYPE_SDES).map_err(Error::from));
        let reader = &mut &payload[..];

        let chunks =
            (0..source_count)
                .map(|_| track_try_unwrap!(SdesChunk::read_from(reader).map_err(Error::from)))
                .collect()
        ;
...
impl WriteTo for SourceDescriptionPacket {
    fn write_to<W: Write>(&self, writer: &mut W) -> Result<()> {
        ...
        for chunk in self.chunks.iter() {
            track!(chunk.write_to(&mut payload).map_err(Error::from));
        }
...
impl ReadFrom for SdesChunk {
    fn read_from<R: Read>(reader: &mut R) -> Result<Self> {
        ...
        loop {
            let ty = track_try_unwrap!(reader.read_u8().map_err(Error::from));
            read_bytes += 1;

            if ty == SDES_ITEM_TYPE_END {
                break;
            }
            let len = track_try_unwrap!(reader.read_u8().map_err(Error::from)) as usize;
            let text = track_try_unwrap!(reader.read_string(len).map_err(Error::from));
        ...
        track!(reader.read_bytes(padding_len as usize).map_err(Error::from)); // discard
...
impl WriteTo for SdesChunk {
    fn write_to<W: Write>(&self, writer: &mut W) -> Result<()> {
        ...
        track!(writer.write_u32be(self.ssrc_or_csrc).map_err(Error::from));
        write_bytes += 4;

        for item in self.items.iter() {
            track!(writer.write_u8(item.item_type()).map_err(Error::from));
            write_bytes += 1;

            let text = item.text();
            track_assert!(text.len() <= 0xFFFF, ErrorKind::Invalid);
            // track!(writer.write_u16be(text.len() as u16));
            track!(writer.write_u8(text.len() as u8).map_err(Error::from));

            track!(writer.write_all(text.as_bytes()).map_err(Error::from));
            // write_bytes += 2 + text.len();
            write_bytes += 1 + text.len();
        }
        track!(writer.write_u8(SDES_ITEM_TYPE_END).map_err(Error::from));
        write_bytes += 1;
...
mod tests {
    ...
    use crate::proto::rtcp::source_description_packet::{SdesChunk, SdesItem};
