...
impl Version {
    ...
    pub fn try_decode(value: &[u8]) -> Result<Self, VersionDecodeError> {
        use self::Version::*;
        ...
        if value.len() != 8
            ...
            || value[6] != b'.'
        {
            return Err(VersionDecodeError::Invalid);
        }

        let major = value[5].checked_sub(b'0').ok_or(VersionDecodeError::Invalid)?;
        let minor = value[7].checked_sub(b'0').ok_or(VersionDecodeError::Invalid)?;
        ...
        if major == 1 && minor == 0 {
            ...
            Err(VersionDecodeError::Invalid)
        } else {
            Err(VersionDecodeError::Unknown(major, minor))
        }
    }
}
...
impl Display for VersionDecodeError {
    fn fmt(&self, formatter: &mut Formatter) -> FormatterResult {
        use self::VersionDecodeError::*;
...
impl Error for VersionDecodeError {}

impl From<Infallible> for VersionDecodeError {
...
pub mod tests {
    ...
    fn test_try_decode() {
        ...
        assert_eq!(Version::try_decode(b""), Err(VersionDecodeError::Invalid));
        assert_eq!(Version::try_decode(b"RTSP/a.b"), Err(VersionDecodeError::Invalid));
        assert_eq!(Version::try_decode(b"RTSP/2"), Err(VersionDecodeError::Invalid));
        assert_eq!(
            Version::try_decode(b"RTSP/9.9"),
            Err(VersionDecodeError::Unknown(9, 9))
        );
        assert_eq!(
            Version::try_decode(b"rtsp/0.0"),
            Err(VersionDecodeError::Unknown(0, 0))
        );
    }
}
