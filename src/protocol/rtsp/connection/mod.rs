    ...
    TTransport: AsyncRead + AsyncWrite + Send + 'static,
{
        ...
        TService::Response: Into<Response<BytesMut>>,
    {
        ...
        let (tx_codec_event, rx_codec_event) = unbounded::<CodecEvent>();
        let (tx_incoming_request, rx_incoming_request) =
            mpsc::channel::<(CSeq, Request<BytesMut>)>(config.request_buffer_size());
        let (tx_pending_request, rx_pending_request) = unbounded::<PendingRequestUpdate>();
    ...
    TTransport: AsyncRead + AsyncWrite + Send + 'static,
{
    ...
    /// task.
    ///
    /// If `Poll::Ready(Ok(()))` is returned, then the connection is shutdown.
    ///
    /// If `Poll::Pending` is returned, then there is nothing else to process for the
...
impl Future for ConnectionShutdownReceiver {
    ...
    ///
    /// If `Poll::Ready(Ok(()))` is returned, then both are shutdown.
    ///
...
mod test {
    ...
    fn test_bounds() {
        ...
        check_send::<Sender<SplitSink<Framed<TcpStream, Codec>, Message>>>();
