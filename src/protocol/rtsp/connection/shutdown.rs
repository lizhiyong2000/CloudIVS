...
impl ShutdownHandler {
    ...
    ///
    /// If `Poll::Ready(Ok(()))` is returned, this implies that a shutdown was initiated as
    /// described above.
    ///
    /// If `Poll::Pending` is returned, then the current task will be notified when another
    /// poll needs to occur.
    ///
    ...
    /// shutdown.
    ///
    /// If `Poll::Ready(Ok(()))` is returned, this implies that the timer has expired which will
...
impl Future for ShutdownHandler {
    ...
    ///
    /// If `Ok(Poll::Ready(ShutdownState))` is returned, this implies that the shutdown state needs
