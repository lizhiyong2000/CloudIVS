...
use crate::protocol::rtsp::protocol::codec::decoder::request::DecodeError as RequestDecodeError;
use crate::protocol::rtsp::protocol::codec::{CodecEvent, DecodeError, Message, ProtocolError};
use crate::protocol::rtsp::protocol::connection::pending::{PendingRequestResponse, PendingRequestUpdate};
use crate::protocol::rtsp::protocol::connection::sender::SenderHandle;
...
where
    TStream: Sink<Message, Error = ProtocolError> + Send + 'static,
{
    ...
    TStream: Sink<Message, Error = ProtocolError> + Send + 'static,
{
    ...
    ///
    /// If `Poll::Ready(())` is never returned.
    ///
    ...
    pub fn poll_codec_events(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<()> {
        loop {
                ...
                .expect("`Receiver.rx_codec_event` should not error")
            {
                ...
                Poll::Ready(None) => panic!("`Receiver.rx_codec_event` should not end"),
            }
        }
    }
    ...
    ///
    /// If `Poll::Ready(())` is returned, then the timer has expired.
    ///
    ...
    ///
    /// If `Poll::Ready(())` is returned, then all of receiving is shutdown.
    ///
    /// If `Poll::Pending` is returned, then receiving is still running, at least partially.
    ...
    ///
    /// If `Poll::Ready(())` is returned, then the stream has been closed and no more messages
    /// will be received.
    ///
    /// If `Poll::Pending` is returned, then either there are no more messages to be processed
    ...
    TStream: Sink<Message, Error = ProtocolError> + Send + 'static,
{
    ...
    ///
    /// If `Poll::Ready(())` is returned, then the receiver (including forwarding) is shutdown.
    ///
    /// If `Poll::Pending` is returned, then there is no more progress that can be made
    /// currently.
    ///
...
impl Future for ForwardingReceiver {
    ...
    ///
    /// If `Poll::Ready(())` is returned, then all requests that could have been forwarded have
    /// been forwarded.
    ///
...
impl Future for ResponseReceiver {
    ...
    ///
    /// If `Poll::Ready(())` is returned, then the pending request update stream has ended and
