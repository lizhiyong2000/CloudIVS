    ...
    TService::Response: Into<Response<BytesMut>>,
{
    ...
    ///
    /// If `Poll::Ready(())` is returned, then either a 100 (Continue) response was sent (and
    ...
    ///
    /// If `Poll::Ready(())` is returned, then the request is finished being serviced. This
        ...
        match self.serviced_request.as_mut() {
            Some((cseq, serviced_request)) => {
                ...
                match serviced_request.poll() {
                    ...
                    Poll::Pending => {
                        ready!(self.poll_continue_timer(cseq));
                        Poll::Pending
                    }
                    Err(_) => {
    ...
    TService::Response: Into<Response<BytesMut>>,
{
    ...
    ///
    /// If `Poll::Ready(())` is returned, this implies that the stream of incoming requests has
