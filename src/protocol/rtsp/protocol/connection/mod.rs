    ...
    TTransport: AsyncRead + AsyncWrite + Send + 'static,
{
    ...
    /// task.
    ///
    /// If `Poll::Ready(())` is returned, then the connection is shutdown.
    ///
...
impl Future for ConnectionShutdownReceiver {
    ...
    ///
    /// If `Poll::Ready(())` is returned, then both are shutdown.
    ///
...
mod test {
    ...
    use crate::protocol::rtsp::protocol::connection::pending::SendRequest;
    use crate::protocol::rtsp::protocol::connection::receiver::Receiver;
    use crate::protocol::rtsp::protocol::connection::sender::{Sender, SenderHandle};
    use crate::protocol::rtsp::protocol::connection::{Connection, ConnectionHandle, ConnectionShutdownReceiver};
