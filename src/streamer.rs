use common::clock::{Clocks, TimerGuard};
use crate::h264;
use crate::stream;
// use storage::{Camera, Stream, dir, recording, writer};
// use storage::{dir};
...
impl<'a, C, S> Streamer<'a, C, S> where C: 'a + Clocks + Clone, S: 'a + stream::Stream {
    pub fn new<'b>(env: &Environment<'a, 'b, C, S>,
...
mod tests {
    ...
    fn basic() {
        ...
        let stream = stream::FFMPEG.open(stream::Source::File("testdata/clip.mp4")).unwrap();
        ...
        let mut stream;
        {
            ...
            stream = super::Streamer::new(&env,
                                          TEST_STREAM_ID, camera, s, 0, 3).unwrap();
        }
        stream.run();
